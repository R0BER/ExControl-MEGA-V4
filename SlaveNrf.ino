...
RF24 radio(9,10);
//RF24 radio(47,49);                              // CE & CS pins to use (Using 7,8 on Uno,Nano)
...
unsigned short OldTotalInput[8];
...
const char HeadChar ='X'; 
const uint16_t dest_address =02; 
...
void loop()
{
  ...
  InputState();  
  for  (int c=0;c<Number_Input;c++){TotalInput[c]=Inputs[c].Value;}
  GestCom();
...
void GestCom(){
  ...
  while ( network.available() )  {                      // Is there anything ready for us?
      ...
      if (header.type=='O'){
        #ifdef EXC_DEBUG_MODE        
          Serial.println("Recepción datos");   
        #endif
        TimRcv=0;
        network.read(header,Output,OutputSize);}
    ...
    for (int p=0;p<8;p++){if (OldTotalInput[p]!=TotalInput[p]){SendData();break;}}
...
void SendData(){
  ...
  if (Escucha){
     ...
     #endif
     RF24NetworkHeader header(dest_address, HeadChar);
     boolean Rt = network.write(header,TotalInput,TotalInputSize);
     if (Rt){for (int k=0;k<8;k++){OldTotalInput[k]=TotalInput[k];}LostPacket=0;}
     else{ActivarEscucha();if (LostPacket>50){delay(1000);IniciarNrf();}}
     #ifdef EXC_DEBUG_MODE        
       if (Rt){Serial.println("ENVIO OK");}else{Serial.println("COLISION?");}
     #endif
}
void ActivarEscucha(){
  TimSend=millis();
...
void ExcTimer(){
  ...
  CountMsg++;
  if (CountMsg>=10){
    CountMsg=0;CountRfSg++;CountSg++;LoopNewSecond();
    #ifdef THERMOSTAT_DS18B20_NUMBER
